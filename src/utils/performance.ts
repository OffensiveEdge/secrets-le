...
export interface PerformanceTracker {
  ...
  end(outputSize: number, itemCount: number, errors: number, warnings: number): PerformanceMetrics
}
...
export function checkPerformanceThresholds(
  ...
  if (metrics.duration > maxDuration) {
    violations.push(`Duration ${metrics.duration.toFixed(2)}ms exceeds ${maxDuration}ms`)
  }

  if (metrics.memoryUsage > maxMemory) {
    violations.push(`Memory ${formatBytes(metrics.memoryUsage)} exceeds ${formatBytes(maxMemory)}`)
  }
