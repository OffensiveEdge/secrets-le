...
export function registerDetectCommand(
  ...
  const disposable = vscode.commands.registerCommand('secrets-le.detect', async () => {
    deps.telemetry.event('detect-command-invoked')

    const editor = vscode.window.activeTextEditor
    if (!editor) {
      deps.notifier.showWarning(
        localize('runtime.detect.no-editor', 'No active editor. Please open a file first.'),
      )
      return
    }
    ...
    try {
      await deps.notifier.showProgress(
        ...
        async (progress, _token) => {
          ...
          const perfTracker = deps.performanceMonitor.startOperation('detect', content.length)
          ...
          if (config.copyToClipboardEnabled) {
            ...
            if (config.notificationsLevel === 'all') {
              deps.notifier.showInfo(
                localize('runtime.detect.copied', 'Results copied to clipboard'),
              )
            }
          }
          ...
          if (secrets.length > 0) {
            if (config.notificationsLevel === 'all') {
              deps.notifier.showWarning(
                localize('runtime.detect.found', 'Found {0} potential secret(s)', secrets.length),
              )
            ...
            deps.notifier.showInfo(
              localize('runtime.detect.none', 'No secrets detected in this document'),
            )
          }
        },
      )
      ...
      deps.notifier.showError(
        localize('runtime.detect.error', 'Detection failed: {0}', errorMessage),
      )
